...
namespace XCommon.CodeGerator.DataBaseReader
{
	internal class DataBaseRead
    {
		internal List<ItemGroup> ReadDataBase()
		{
		...
		private List<Item> GetItems(SqlConnection cnx, List<DataBaseObject> dataBaseObjects, string schema)
		{
			...
			foreach (var tableItem in dataBaseObjects.Where(c => c.Schema == schema).Select(c => c.TableName).OrderBy(c => c).Distinct())
			{
				...
				{
					Name = tableItem,
					Properties = GetProperties(dataBaseObjects, schema, tableItem),
		...
		private List<ItemProperty> GetProperties(List<DataBaseObject> dataBaseObjects, string schema, string table)
		{
			...
			{
				var remap = Generator.Configuration.DataBase.Remap.FirstOrDefault(c => c.Schema == schema && c.Table == table && c.Column == column.ColumnName);
		...
		private List<DataBaseObject> GetDataBaseObjects(SqlConnection cnx)
		{
			List<DataBaseObject> result = new List<DataBaseObject>();

			SqlCommand cmd = new SqlCommand(SqlDataObjects(), cnx);
